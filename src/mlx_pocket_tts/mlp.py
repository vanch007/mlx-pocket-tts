...
def _rms_norm(x: mx.array, alpha: mx.array, eps: float) -> mx.array:
    ...
    width = max(1, x.shape[-1] - 1)
    var = eps + mx.sum(mx.square(x - mean), axis=-1, keepdims=True) / width
...
class LayerNorm(nn.Module):
    """PyTorch-compatible LayerNorm with higher-order MLX gradients.

    The fused MLX kernel is ideal for inference, but currently cannot be used
    beneath the JVP/VJP composition required by the LSD training objective.
    """
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        value = (value - mean) * mx.rsqrt(variance + self.eps)
        if self.weight is not None:
            value = value * self.weight.astype(mx.float32)
        if self.bias is not None:
            value = value + self.bias.astype(mx.float32)
        return value.astype(input_dtype)


class SiLU(nn.Module):
    """Unfused SiLU so LSD can take a VJP through its JVP."""
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self.freqs = mx.exp(-math.log(max_period) * mx.arange(0, half, dtype=mx.float32) / half)
        self.mlp = [
