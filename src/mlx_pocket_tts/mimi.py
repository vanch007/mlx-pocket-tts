...
from .codec.conv import (
    Conv1d,
    ConvDownsample1d,
    ConvTrUpsample1d,
    get_extra_padding_for_conv1d,
)
from .codec.seanet import (
    SeanetConfig,
    SeanetDecoder,
    SeanetEncoder,
)
from .codec.transformer import (
...
class MimiAdapter(nn.Module):
    def __init__(
        ...
        configured_inner_dim = inner_dim or self.dimension
        configured_outer_dim = outer_dim or self.dimension
        ...
        if encoder_frame_rate != frame_rate:
            ...
            self.downsample = ConvDownsample1d(
                downsample_stride,
                dim=self.dimension,
                out_dim=configured_inner_dim,
                causal=True,
            )
            self.upsample = ConvTrUpsample1d(
                downsample_stride,
                dim=self.dimension,
                in_dim=configured_outer_dim,
                causal=True,
            )
        else:
    ...
    def encode_to_latent(self, x: mx.array) -> mx.array:
        if x.ndim != 3:
            raise ValueError("MimiAdapter.encode_to_latent expects audio of shape [B, C, T].")
        self.encoder.reset_state()
        for cache in self.encoder_cache:
            _reset_kv_cache(cache)
        if self.downsample is not None:
