...
class StreamableConv1d(nn.Module):
    ...
    def step(self, xs: mx.array) -> mx.array:
        ...
        if not self._left_pad_applied:
            ...
            xs = mx.pad(xs, pad_width=((0, 0), (0, 0), (padding_total, 0)), mode=self._pad_mode)
...
class ConvDownsample1d(nn.Module):
    def __init__(self, stride: int, dim: int, causal: bool, out_dim: int | None = None):
        super().__init__()
...
class ConvTrUpsample1d(nn.Module):
    def __init__(self, stride: int, dim: int, causal: bool, in_dim: int | None = None):
        super().__init__()
