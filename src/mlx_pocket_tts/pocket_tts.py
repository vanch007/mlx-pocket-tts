...
from .text_chunking import (
    prepare_text_prompt as official_prepare_text_prompt,
)
from .text_chunking import (
...
class Model(nn.Module):
    def __init__(self, config: ModelConfig | dict[str, Any]):
        ...
        self.temp = config.default_temperature or DEFAULT_TEMPERATURE
        ...
        self.speaker_proj_weight = mx.zeros(
            (
                ...
                config.mimi.inner_dim or config.mimi.quantizer.output_dimension,
            ),
            dtype=mx.float32,
        )
    ...
    @property
    def device(self) -> str:
        """MLX executes this model on the Apple Metal device."""
        return "metal"

    def to(self, device: str) -> "Model":
        """Accept the official device call while validating the MLX platform boundary."""
        if str(device).lower() not in {"metal", "mps", "gpu", "cpu"}:
            raise ValueError(f"MLX Pocket TTS only supports Apple Metal; got {device!r}.")
        return self
    ...
    def load_model(
        ...
        sampler_decode_steps: int = DEFAULT_LSD_DECODE_STEPS,
    ...
    def _cached_get_state_for_audio_prompt(
        ...
        key = (str(audio_conditioning), truncate)
        if key not in self._voice_state_cache:
            if len(self._voice_state_cache) >= 2:
                self._voice_state_cache.pop(next(iter(self._voice_state_cache)))
            self._voice_state_cache[key] = self.get_state_for_audio_prompt(
                audio_conditioning, truncate=truncate
            )
        return self._voice_state_cache[key]

    def get_state_for_audio_prompt(
        self, audio_conditioning: mx.array | Path | str, truncate: bool = False
    ):
        if isinstance(audio_conditioning, (str, Path)):
            source_text = str(audio_conditioning)
            suffix_text = source_text.split("@", 1)[0]
            if suffix_text.endswith(".safetensors"):
                return self._load_voice_file(download_if_necessary(source_text))
        local_voice = self._local_voice_path(audio_conditioning)
        if local_voice is not None:
            loaded = self._load_voice_file(local_voice)
            ...
            if not self.has_voice_cloning:
                raise ValueError(
                    "This artifact contains zeroed public encoder weights and cannot clone "
                    "an arbitrary reference voice. Use a bundled preset voice or a gated "
    ...
    def _local_voice_path(self, source: mx.array | Path | str) -> Path | None:
        if self.model_path is None or not isinstance(source, (str, Path)):
            return None
        text = str(source)
        direct = Path(text).expanduser()
        if direct.is_file() and direct.suffix == ".safetensors":
            return direct
    ...
    def _load_voice_file(self, path: Path) -> mx.array | dict[str, Any]:
        ...
        caches = self.flow_lm.make_cache()
        for index, cache in enumerate(caches):
            prefix = f"transformer.layers.{index}.self_attn/"
            packed = tensors.get(prefix + "cache")
            if packed is None:
                raise ValueError(f"Voice state is missing {prefix}cache")
            cache.keys = packed[0].transpose(0, 2, 1, 3)
            cache.values = packed[1].transpose(0, 2, 1, 3)
            offset = tensors.get(prefix + "offset")
            cache.offset = int(offset[0].item()) if offset is not None else cache.keys.shape[2]
        return {"flow_cache": caches}
    ...
    def generate_audio_stream(
        ...
        if model_state is None:
            ...
            model_state = self._copy_model_state(model_state)
        prompt_num_frames = self._get_flow_cache_num_frames(model_state)
        chunks = official_split_into_best_sentences(
            self.flow_lm.conditioner.tokenizer,
            text_to_generate,
            max_tokens,
            self.config.pad_with_spaces_for_short_inputs,
            self.config.remove_semicolons,
        )
        for chunk in chunks:
            self._slice_flow_cache(model_state, prompt_num_frames)
            prepared_chunk, frames_after_eos_guess = official_prepare_text_prompt(
                chunk,
                self.config.pad_with_spaces_for_short_inputs,
                self.config.remove_semicolons,
            )
            effective_frames_after_eos = (
                frames_after_eos
                if frames_after_eos is not None
                else (self.config.model_recommended_frames_after_eos or frames_after_eos_guess + 2)
            )
            yield from self._generate_audio_stream_short_text(
                model_state=model_state,
                text_to_generate=prepared_chunk,
                frames_after_eos=effective_frames_after_eos,
            )

    def _copy_model_state(self, model_state: dict[str, Any]) -> dict[str, Any]:
        copied = self.init_state()
        for source, target in zip(
            model_state.get("flow_cache", []), copied["flow_cache"], strict=True
        ):
            if source.keys is None:
                continue
            target.keys = mx.array(source.keys[..., : source.offset, :])
            target.values = mx.array(source.values[..., : source.offset, :])
    ...
    def _generate_audio_stream_short_text(
        ...
        self._run_flow_lm_and_increment_step(model_state=model_state, text_tokens=prepared.tokens)

        backbone_input = mx.full((1, 1, self.flow_lm.ldim), float("NaN"), dtype=mx.float32)
        eos_step = None
    ...
    def _resolve_audio_prompt(
        ...
        if isinstance(prompt, str):
            ...
            if verbose:
                print(
                    f"Voice '{prompt}' not found for pocket_tts; "
                    f"using '{DEFAULT_AUDIO_PROMPT}' instead."
                )
...
def _build_generation_result(
    ...
    real_time_factor = audio_duration_seconds / elapsed_time if elapsed_time > 0 else 0.0
    ...
    return GenerationResult(
        ...
        prompt={
            "tokens": token_count,
            "tokens-per-sec": (round(token_count / elapsed_time, 2) if elapsed_time > 0 else 0.0),
        },
        audio_samples={
            "samples": samples,
            "samples-per-sec": (round(samples / elapsed_time, 2) if elapsed_time > 0 else 0.0),
        },
